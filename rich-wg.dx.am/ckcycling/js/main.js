...
let photos = [];
let files = {};
const user = { permissions: [], name: null, id: null };
...
function setOptions(url) {
    ...
    fetch(url)
        ...
        .then(t => {
            const firstTour = 'CST';
            let fileList = JSON.parse(t);
            firstIndex = fileList.indexOf(fileList.find(x => x.name === firstTour));
            const firstElement = fileList.splice(firstIndex, firstIndex)[0];
            fileList.unshift(firstElement);
...
function calcInfo() {
    ...
    else enddate = "<br> to " + enddate;
    for (let track of tracks) {
        distance += track.distance.total / 1000;
        elevation += track.elevation.pos;
        negativeElevation += track.elevation.neg;
        let dt = track.points[track.points.length - 1].time - track.points[0].time;
        time += dt / millisPerHour;
        for (let k = 1; k < track.points.length; k++) {
            let dtp = track.points[k].time - track.points[k - 1].time;
            if (dtp < 30000) {
                movementTime += dtp;
            }
        }
    }
    movementTime /= millisPerHour;
    info.innerHTML = `<table class="w100">
        <tr>
            <th colspan="2">Tour Info</th>
        </tr>
        <tr>
            <td>
                Date
        ...
        <tr>
            ...
            <td>
                ${elevation.toFixed(2).toString()}m
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td>
                ${negativeElevation.toFixed(2).toString()}m
            </td>
        </tr>
        <tr>
            <td>
        ...
        <tr>
            ...
            <td>
                ${(distance/time).toFixed(2).toString()}km/h
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td>
                ${movementTime.toFixed(2).toString()}h
            </td>
        </tr>
        <tr>
            <td>
            ...
            <td>
                ${(distance/movementTime).toFixed(2).toString()}km/h
            </td>
        </tr>
        </table>`;
}
